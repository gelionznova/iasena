...
#         user = self.request.user
#         if user.is_superuser:
#             return MeetingSession.objects.all()
#         return MeetingSession.objects.filter(meeting__user=user)
...
#         serializer.is_valid(raise_exception=True)
#         meeting = serializer.validated_data['meeting']
#         if meeting.user != request.user and not request.user.is_superuser:
#             raise PermissionDenied("No tienes permiso para agregar sesiones a esta reunión.")
#         serializer.save()
#         return Response(serializer.data, status=status.HTTP_201_CREATED)
...
#         user = self.request.user
#         if user.is_superuser:
#             qs = MeetingPhoto.objects.all()
#         else:
#             qs = MeetingPhoto.objects.filter(meeting__user=user)
...
#     def get_queryset(self):
#         user = self.request.user
#         qs = MeetingSessionActaVersion.objects.all() if user.is_superuser else MeetingSessionActaVersion.objects.filter(session__meeting__user=user)
